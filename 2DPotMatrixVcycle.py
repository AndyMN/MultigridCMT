...
### EXACT VALUES ###################################
####################################################
nx = [1, 2, 1, 2, 3, 1, 2, 3, 1, 4]
ny = [1, 1, 2, 2, 1, 3, 3, 2, 4, 1]
n = zip(nx, ny)
...
w0 = np.zeros((gridsize**2, num_eigenvals))
...
while iters < max_iters:
    iters += 1
    w = w0
    w = solver.vcycle_matrix(w, eigenvectors_MG, hamiltonian, stencil_maker, shifts=bad_eigenvals, dimension="2d", lowest_level=2**3)
    for i in xrange(num_eigenvals):
        eigenvectors_MG[:, i] = w[:, i] / np.linalg.norm(w[:, i])
        ...
        #errors[iters, i] = np.linalg.norm(eigenvectors_MG[:, i] - hamiltonian.dot(w[:, i]))
        shifted_matrix = hamiltonian - sparse.eye(gridsize**2) * bad_eigenvals[i]  # Create the shifted matrix A-mu*I
...
for i in xrange(num_eigenvals):
    if i != 0:
        if i % 2 == 0:
            ...
            column_index += 1
    axarray[row_index, column_index].plot(iter_x, errors[:, i] / errors[0, i])
