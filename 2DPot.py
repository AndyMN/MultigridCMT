...
### EXACT VALUES ###################################
####################################################
nx = [1, 1, 2, 2, 3, 1, 2, 3, 4, 1]
ny = [1, 2, 1, 2, 1, 3, 3, 2, 1, 4]
n = zip(nx, ny)
...
for i in xrange(num_eigenvals):
    ...
    eigenvalues[0, i] = bad_eigenvals[i]
...
"""
v = v.reshape(gridsize, gridsize)
Psi = Psi.reshape(gridsize, gridsize)

fig_exact = pl.figure(1)
ax_exact = fig_exact.gca(projection="3d")
ax_exact.plot_surface(X, Y, Psi * Psi)



fig_mg = pl.figure(2)
ax_mg = fig_mg.gca(projection="3d")

ax_mg.plot_surface(X, Y, v.T * v)
