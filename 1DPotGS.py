...
while iters < max_iters:
    ...
    for j in xrange(num_eigenvalues):
        w = w0
        w = solver.vcycle(w, eigenvectors_shift[:, j], hamiltonian, stencil_maker, shift=bad_eigenvalues[j], lowest_level=2**4)
        eigenvectors_shift[:, j] = w / np.linalg.norm(w)
    ...
    eigenvectors_shift = processor.gramschmidt(eigenvectors_shift)
